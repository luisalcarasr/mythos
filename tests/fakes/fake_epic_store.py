...
"""In-memory fake implementation of EpicStorePort for testing."""
...
class FakeEpicStore(EpicStorePort):
    """
    Deterministic, in-memory Epic store for unit tests.

    Seed it with ``games`` and ``installed`` dicts keyed by AppName.
    """
    ...
    def install_game(
        ...
        on_progress(Progress(fraction=0.5, downloaded_bytes=512, total_bytes=1024))
        on_progress(Progress(fraction=1.0, downloaded_bytes=1024, total_bytes=1024))
    ...
    def move_game(self, app_name: AppName, new_path: Path) -> InstalledInfo:
        info = self._installed[app_name]
        updated = InstalledInfo(
            app_name=info.app_name,
            install_path=InstallPath(new_path),
